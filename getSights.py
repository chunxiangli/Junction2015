...
def getData():
    ...
    for city in citys:
        url = "https://api.foursquare.com/v2/venues/search?client_id=QZN5SN0P5Q0DFENZZCJHQ30UXQFUO11IFBTHHKUTW1I4PLUC&client_secret=1MOZSBPTWOLEQQBMD0ZT2G3YQKC2PJUHSPGOFOEAYQIO1XTD&near=%s&categoryId=%s&v=20140806&m=foursquare" % (city, restaurant)
        response = urllib.urlopen(url).read()
        data = json.loads(response)
        with open("restaurants/%s.json" % city, "w") as outfile:
...
def getRestaurantsByCity(city):
    ...
    citydata = json.loads(open("restaurants/%s.json" % city).read())
    attractions = []
    for venue in citydata["response"]["venues"]:
        oneplace = {}
        oneplace["name"] = venue["name"]
...
if __name__ == "__main__":
    ...
    print getRestaurantsByCity("Paris")
    pass
